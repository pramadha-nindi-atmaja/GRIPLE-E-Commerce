...
export async function getCategoriesByGender(gender: "men" | "women"): Promise<Category[]> {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/categories?gender=${gender}`, {
      cache: 'no-store',
    });

    if (!response.ok) {
      console.error('Failed to fetch categories by gender:', response.statusText);
      return [];
    }
