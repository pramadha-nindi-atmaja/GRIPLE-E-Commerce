...
export function FilterSidebar({ categories, colors, sizes }: Props) {
  ...
  const toggleGender = (value: "men" | "women" | "unisex") => {
    ...
    replaceQuery({
      ...query,
      gender: next as ParsedStoreQuery["gender"],
    });
  };
  ...
  const toggleCategory = (slug: string) => {
    ...
    replaceQuery({
      ...query,
      category: toggleStr(query.category, slugLower),
    });
  };
  ...
  const toggleSize = (size: string) => {
    replaceQuery({
      ...query,
      size: toggleStr(query.size, size),
    });
  };
  ...
  const toggleColor = (name: string) => {
    replaceQuery({
      ...query,
      color: toggleStr(query.color, name),
    });
  };
  ...
  return (
    <aside className="w-full md:w-[260px] shrink-0 border-r border-outline-variant pr-gutter pb-12 hidden md:block">
      <div className="flex justify-between items-center mb-8 gap-2">
        ...
        <button
          type="button"
          className="font-label-caps text-label-caps text-on-surface-variant hover:text-primary underline underline-offset-4 rounded-full shrink-0"
          onClick={clearAll}
      ...
      <details open className="border-b border-outline-variant py-4 group">
        ...
        <FilterPriceInputs
          ...
          onApply={(minPrice, maxPrice) =>
            replaceQuery({ ...query, minPrice, maxPrice })
          }
        />
      </details>
    </aside>
